...
# CHANGE 4: Return detailed scoring explanation

def score_assessment_detailed(session_id, responses_dict):
    ...
    # Step 1: Raw scores
    raw_scores = calculate_raw_scores(responses_dict)
    result['steps']['step_1'] = {
        'name': 'Raw Domain Scores',
        'raw_scores': raw_scores,
        'details': f"Averaged {len(responses_dict)//6} items per domain"
    }
    
    # Step 2: Validity/Lambda
    lambda_value = calculate_lambda(responses_dict, raw_scores)
    result['steps']['step_2'] = {
        'name': 'Deception Susceptibility (Λ)',
        'lambda': lambda_value,
        'interpretation': 'Low' if lambda_value < 0.5 else 'Moderate' if lambda_value < 1.0 else 'High'
    }
...
# CHANGE 3: Cache intermediate results

import hashlib
from functools import lru_cache

@lru_cache(maxsize=1000)
def calculate_domain_score_cached(domain_responses_tuple, domain_code):
    """Cache domain score calculations"""
    domain_responses = list(domain_responses_tuple)
    return np.mean(domain_responses)

# Or database caching:
def get_or_calculate_scores(session_id, responses_dict):
    ...
    cursor.execute(
        "SELECT raw_scores FROM scoring_cache WHERE session_id = %s",
        (session_id,)
    )
    cached = cursor.fetchone()
    
    if cached:
        logger.info(f"Using cached scores for {session_id}")
    ...
    # Cache result
    cursor.execute(
        "INSERT INTO scoring_cache (session_id, raw_scores, created_at) VALUES (%s, %s, %s)",
        (session_id, json.dumps(scores), get_current_timestamp())
    )
    conn.commit()
...
# CHANGE 2: Ensure numerical stability

import numpy as np

def ensure_numerical_stability(scores):
    """Check for NaN, Inf, or out-of-range values"""
    
    # Check for NaN
    if np.isnan(scores).any():
        logger.error("NaN detected in scores")
        # Replace with mean
        scores = np.nan_to_num(scores, nan=5.0)
    
    # Check for Inf
    if np.isinf(scores).any():
        logger.error("Infinity detected in scores")
...
# Use in Step 2 and Step 5:
raw_scores = ensure_numerical_stability(raw_scores)
corrected_scores = ensure_numerical_stability(corrected_scores)
...
# CHANGE 1: Validate input before processing

def validate_responses(responses_dict):
    """Validate raw responses before scoring"""
    issues = []
    
    # Check all 35 items present
    required_items = ['R1', 'R2', 'R3', 'R4', 'R5',
                      'S6', 'S7', 'S8', 'S9', 'S10',
                      'C11', 'C12', 'C13', 'C14', 'C15',
                      'A16', 'A17', 'A18', 'A19', 'A20',
                      'O21', 'O22', 'O23', 'O24', 'O25',
    ...
    for item in required_items:
        if item not in responses_dict:
            issues.append(f"Missing response for {item}")
        else:
            value = responses_dict[item]
            if not isinstance(value, (int, float)) or value < 0 or value > 10:
                issues.append(f"Invalid value for {item}: {value} (must be 0-10)")
    
    if issues:
        raise ValueError(f"Response validation failed: {', '.join(issues)}")
    
    return True

# Use in scoring function:
def score_assessment(session_id, responses_dict):
    """Score assessment with validation"""
    try:
        validate_responses(responses_dict)
