...
# CHANGE 5: Function to aggregate all assessment data

def get_complete_results(session_id):
    """Get all assessment data in one query"""
    
    conn = get_db_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    
    try:
        # Multi-join query for efficiency
        cursor.execute("""
            SELECT
                a.session_id, a.age, a.sex, a.country, a.created_at,
                sr.raw_scores, sr.corrected_scores, sr.lambda, sr.confidence_intervals,
                mr.personality_type, mr.confidence, mr.anomaly_score, mr.latent_vector,
                n.narrative_text, n.generated_at
            FROM assessments a
            LEFT JOIN scoring_results sr ON a.session_id = sr.session_id
            LEFT JOIN ml_results mr ON a.session_id = mr.session_id
            LEFT JOIN narratives n ON a.session_id = n.session_id
        ...
        result['corrected_scores'] = json.loads(result['corrected_scores'])
...
# CHANGE 4: Monitor connection pool health

class DatabasePool:
    """Connection pool with monitoring"""
    
    def __init__(self, database_url, min_connections=2, max_connections=20):
        self.pool = SimpleConnectionPool(min_connections, max_connections, database_url)
        self.min_connections = min_connections
        self.max_connections = max_connections
        self.stats = {'gets': 0, 'puts': 0, 'errors': 0}
    ...
    def get_pool_stats(self):
        ...
        return {
            ...
            'current_available': self.pool.closed,  # Closed count
...
# CHANGE 3: Add indexes and query optimization hints
...
def ensure_indexes_exist():
    ...
    indexes = [
        ...
        ("CREATE INDEX IF NOT EXISTS idx_scores_session_id ON scoring_results(session_id)",),
        ("CREATE INDEX IF NOT EXISTS idx_ml_results_session_id ON ml_results(session_id)",),
        ("CREATE INDEX IF NOT EXISTS idx_narratives_session_id ON narratives(session_id)",),
    ]
...
# CHANGE 2: Support efficient batch inserts

def batch_insert_responses(session_id, responses_dict, batch_size=100):
    """Insert multiple responses efficiently"""
    ...
    try:
        # Build batch insert
        values = []
        for item_code, score in responses_dict.items():
            values.append((session_id, item_code, int(score), get_current_timestamp()))
        
        # Insert in batches
        query = """
            INSERT INTO responses (session_id, item_code, score, created_at)
            VALUES (%s, %s, %s, %s)
        """
        ...
        conn.commit()
        logger.info(f"Inserted {len(values)} responses for {session_id}")
...
# CHANGE 1: Add context manager for transactions

from contextlib import contextmanager

@contextmanager
def get_db_transaction(auto_commit=True):
